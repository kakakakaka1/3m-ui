...
Use a single author for commits on this repository (GitHub account **kazeyukiro**):

```bash
git config user.name "kazeyukiro"
git config user.email "177553227+kazeyukiro@users.noreply.github.com"
```

Prefer a **local** config inside the clone (`git config` without `--global`) so other projects are unaffected.

Historical commits from automation (`Z User <z@container>`) are mapped via `.mailmap` for `git log` / shortlog display. Do not rewrite published history on `main` unless coordinated.
